        ...
        try {
            ...
            for (int i = 0; i < addresses.length; i++) {
                try {
                    ...
                    log.error("Failed to connect to server '{}'.", addresses[i], ex);
                }
            }
            ...
            if (!reConnecting.get()) {
                ...
                for (int i = 1; i <= 3; i++) {
                    try {
                        ...
                        future.await();
                        if (future.isSuccess() && future.cause() == null) {
                            log.info("尝试第 {} 次重连到 '{}' 成功!", i, curServer);
                            errorFlag.set(false);
                            return;
                        } else {
                        ...
                        log.error("尝试第 {} 次重连到 '{}' 失败!", new Object[] { i, curServer, ex });
                    }
                }
